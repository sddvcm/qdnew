...
> **需要密码登录。** 默认密码是 `123456`，首次打开就能进。
> 登录后请到 **系统设置 → 访问密码** 改成只有你知道的密码 ——
> 若打算通过反代对外网开放，这一步务必做（详见 §2.5）。

### 2.5 访问密码与对外暴露

程序自带一层访问密码保护，**默认开启**，默认密码 `123456`。

| 位置 | 作用 |
|------|------|
| 安装向导（FPK） | 装机时可直接设端口与访问密码 |
| 系统设置 → 访问密码 | 改密码、开关保护（关闭需验当前密码）|

**改动规则：**

- 密码用 `PBKDF2-HMAC-SHA256`（20 万轮 + 随机盐）存 `system_config` 表，**不存明文**。
- 改密码会让**所有已登录会话立刻失效**（浏览器需要重新登录）。
- 只要你还在用默认密码，界面顶栏会一直挂一条黄色提醒（关掉仅在当前页生效）。
- 忘记密码：清掉库里这两行配置即可恢复默认 ——
  `DELETE FROM system_config WHERE key IN ('auth_password_hash','auth_password_salt');`

> ⚠️ **外网反代必须配访问控制。** 虽然程序有密码保护，但它是应用层的；
> 建议同时启用反代自带的 Basic Auth 或 IP 白名单，做双层防护。
